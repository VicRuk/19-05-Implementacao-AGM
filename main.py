...
def main():
    ...
    origem = top_saida[1][0]
    destino = top_saida[2][0]  
    print("\n" + "="*60)
    ...
    # Algoritmo de Prim
    print(f"\nÁrvore Geradora Mínima (Algoritmo de Prim) a partir de {origem}:")
    custo_mst, arestas_mst = grafo.arvore_geradora_minima_prim(origem)
    if arestas_mst:
        print(f"Custo Total da Árvore: {custo_mst}")
        print("Arestas que compõem a Árvore (Origem -> Destino [Peso]):")
        
        # Mostra apenas as 10 primeiras arestas para não poluir o terminal caso o grafo seja enorme
        for u, v, peso in arestas_mst[:10]:
            print(f"   {u} -> {v} [Peso: {peso}]")
            
        if len(arestas_mst) > 10:
            print(f"   ... e mais {len(arestas_mst) - 10} conexões.")
    else:
        print("Não foi possível gerar uma árvore a partir deste nó (Grafo desconexo).")
