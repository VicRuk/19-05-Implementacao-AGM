...
class Grafo:
    ...
    # Retorna o custo total da árvore e a lista de arestas que a compõem.
    def arvore_geradora_minima_prim(self, email_raiz=None):
        if self.num_vertices == 0:
            return 0, []

        INFINITO = float('inf')
        
        # chave[i] guarda o menor peso de aresta que conecta o vértice 'i' à árvore
        chave = [INFINITO] * self.num_vertices
        # pai[i] guarda o vértice que conectou 'i' à árvore
        pai = [-1] * self.num_vertices
        # na_arvore[i] indica se o vértice 'i' já foi incluído na MST
        na_arvore = [False] * self.num_vertices

        # Define a raiz (se não for passada, pega o primeiro vértice inserido no grafo)
        if email_raiz and email_raiz in self.email_para_id:
            raiz = self.email_para_id[email_raiz]
        else:
            raiz = 0
        ...
        for _ in range(self.num_vertices):
            # 1. Busca o vértice 'u' com a menor chave que AINDA NÃO está na árvore
            min_chave = INFINITO
            u = -1
            for i in range(self.num_vertices):
                if not na_arvore[i] and chave[i] < min_chave:
                    min_chave = chave[i]
                    u = i

            # Se o grafo for desconexo ou houver vértices inalcançáveis, interrompe
            if u == -1:
                break

            # 2. Adiciona o vértice 'u' na Árvore Geradora Mínima
            na_arvore[u] = True

            # 3. Atualiza as chaves e os pais dos vizinhos do vértice 'u' recém-adicionado
            for aresta in self.vertices[u].adjacencias:
                v = aresta.destino
                peso = aresta.peso

                # Se o vizinho 'v' não está na árvore e o peso da aresta (u, v) for MENOR do que a melhor conexão que 'v' tinha até agora, atualizamos a conexão.
                if not na_arvore[v] and peso < chave[v]:
                    chave[v] = peso
                    pai[v] = u

        # 4. Monta o resultado final (Calcula o custo e estrutura as arestas)
        custo_total = 0
        arestas_mst = []
        for i in range(self.num_vertices):
            # Se o vértice tem um pai, ele faz parte da MST (ignora a raiz que tem pai -1)
            if pai[i] != -1: 
                custo_total += chave[i]
                origem = self.id_para_email[pai[i]]
                destino = self.id_para_email[i]
                arestas_mst.append((origem, destino, chave[i]))

        return custo_total, arestas_mst